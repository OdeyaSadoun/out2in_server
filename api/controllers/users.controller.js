  ...
  editUser: async (req, res) => {
    ...
    try {
      ...
      if (req.tokenData.role == "admin" || idEdit == req.tokenData._id) {
        ...
        data = await UserModel.updateOne({ _id: idEdit, active: "true" }, req.body);
      } else {
        data = [
          {
            status: "failed",
    ...
    try {
      ...
      if (req.tokenData.role == "admin" || (idDelete == user.idCard&&req.tokenData.role == "principal")) {
        ...
        if (teacherByPrincipal1.length > 0) {
          data = await UserModel.updateOne({ _id: teacherByPrincipal1[0].user_id, active: "true" }, { $set: { "active": false } });
        }
        else {
          data = [
            {
